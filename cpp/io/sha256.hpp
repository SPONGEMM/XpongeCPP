...
public:
  ...
  std::string hex_digest() const {
    Sha256 copy = *this;
    const auto bit_size = static_cast<std::uint64_t>(copy.total_size_) * 8U;
    ...
    for (std::size_t index = 0; index < encoded_size.size(); ++index)
      encoded_size[index] = static_cast<std::uint8_t>(bit_size >> (56U - index * 8U));
