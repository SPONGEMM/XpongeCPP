...
std::string pdb_field_string(const std::string& line, std::size_t pos, std::size_t len) {
    if (line.size() <= pos) {
        return "";
    }
...
Assign get_assignment_from_pdb_text(const std::string& text) {
    ...
    while (std::getline(input, line)) {
        if (line.rfind("ATOM", 0) == 0 || line.rfind("HETATM", 0) == 0) {
            const int serial = std::stoi(pdb_field_string(line, 6, 5));
            const std::string name = pdb_field_string(line, 12, 4);
            const std::string element = guess_element(name, pdb_field_string(line, 76, 2));
            const auto atom_id = static_cast<std::uint32_t>(assignment.atom_count());
...
std::string assignment_to_pdb_text(const Assign& assignment, const std::string& residue_name) {
    std::ostringstream out;
    const std::string resname = residue_name.empty() ? assignment.name : residue_name;
    out << "REMARK   Generated By Xponge (Assignment)\n";
    out << std::fixed << std::setprecision(3);
    for (std::uint32_t i = 0; i < assignment.atom_count(); ++i) {
        const auto& coord = assignment.coordinates[i];
        const std::string atom_name = assignment.names[i].empty() ? assignment.elements[i] + std::to_string(i + 1)
                                                                  : assignment.names[i];
        out << "ATOM  " << pdb_int_field(5, static_cast<int>(i + 1)) << " " << std::left << std::setw(4)
            << atom_name.substr(0, 4) << std::right << " " << std::setw(3) << resname.substr(0, 3) << " "
            << " " << pdb_int_field(4, 1) << "    " << std::setw(8) << coord[0] << std::setw(8) << coord[1]
            << std::setw(8) << coord[2] << "                      " << std::setw(2)
            << assignment.elements[i].substr(0, 2) << "\n";
    }
    for (std::uint32_t i = 0; i < assignment.atom_count(); ++i) {
        std::vector<std::uint32_t> bonded_atoms;
        bonded_atoms.reserve(assignment.bonds[i].size());
        for (const auto& [neighbor, order] : assignment.bonds[i]) {
            (void)order;
            bonded_atoms.push_back(neighbor);
        }
        std::sort(bonded_atoms.begin(), bonded_atoms.end());
        for (std::size_t offset = 0; offset < bonded_atoms.size(); offset += 4) {
            out << "CONECT" << pdb_int_field(5, static_cast<int>(i + 1));
            for (std::size_t j = offset; j < std::min(offset + 4, bonded_atoms.size()); ++j) {
                out << pdb_int_field(5, static_cast<int>(bonded_atoms[j] + 1));
            }
            out << "\n";
        }
    }
