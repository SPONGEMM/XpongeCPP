...
| RESP | Partial | Default routing prefers an installed `PySCF`, automatically falls back to an installed `Psi4`, and reports a clear backend-unavailable error when neither exists; explicit backend selection remains strict. Python/C++ RESP-core parity tests exist. Real-fixture regressions pass under `PySCF`, while `Psi4` coverage is currently guarded by optional-dependency tests rather than mandatory CI. Full large-case and all-parameter Xponge parity is not complete. |
